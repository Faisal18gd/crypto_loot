...
class PassReset extends Controller
{
    ...
    public function reset(Request $req)
    {
        ...
        if (!$check) {
            return back()->with('error', 'Account not found with this email');
        }
        if ($check->banned == 'yes') {
    ...
    public function apiReset(Request $req)
    {
        ...
        try {
            $email = \EncDec::dec($req->json('d'));
            $check = DB::table('users')->where('email', $email)->first();
            if (!$check) {
                return response("User not found!", 400);
            }
            if ($check->banned == 'yes') {
                return response("User has been banned!", 400);
            }
            if (Cache::has('reset-' . $email)) {
                return response('We have already sent you an email with a validation link', 400);
            } else {
                $key = $this->generateRandomString();
                Cache::put('reset-' . $email, 1, 60);
                ...
                return ['status'=>'We have sent you an email with a validation link. Please confirm with that link'];
            }
        } catch (\Exception $e) {
            return response('Error occurred', 400);
        }
    }
    ...
    public function makeReset($key = null)
    {
        if ($key && Cache::has($key)) {
            ...
            Cache::forget($key);
            return redirect()->action('Auth\PassReset@view')
            ->with('success', 'Password reset was successful, now you can login with your new password.');
        } else {
            return redirect()->action('Auth\PassReset@view')
            ->with('error', 'Either reset validation link expired or invalid reset link!');
        }
    }
